...
export const AppHttpRequests = () => {
  ...
  useEffect(() => {
    ...
    todolistsApi.getTodolists().then((res) => {
      ...
      todolists.forEach((todo) => {
        tasksApi.getTasks(todo.id).then((res) => setTasks((prevState) => ({ ...prevState, [todo.id]: res.data.items })))
      })
    })
  }, [])
  ...
  const changeTaskStatus = (e: ChangeEvent<HTMLInputElement>, task: DomainTask) => {
    tasksApi.changeTaskStatus(e, task).then((res) => {
      setTasks({
        ...tasks,
        [task.todoListId]: tasks[task.todoListId].map((t) => (t.id === task.id ? { ...t, ...res.data.data.item } : t)),
      })
    })
  }

  const changeTaskTitle = (task: DomainTask, title: string) => {
    tasksApi.changeTaskTitle(title, task).then((res) => {
      setTasks({
        ...tasks,
        [task.todoListId]: tasks[task.todoListId].map((t) => (t.id === task.id ? { ...t, ...res.data.data.item } : t)),
      })
